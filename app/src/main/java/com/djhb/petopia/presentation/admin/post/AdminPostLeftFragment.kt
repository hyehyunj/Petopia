...
class AdminPostLeftFragment : DialogFragment() {
    private val _binding: FragmentAdminPostLeftBinding by lazy {
        FragmentAdminPostLeftBinding.inflate(layoutInflater)
    }
    private val binding get() = _binding

    private val adminViewModel:AdminPostViewModel by activityViewModels()
    private val deckPager by lazy { binding.adminPostLeftDeck }
    ...
    private fun adminPostLeftButtonClickListener() {
        //뒤로가기버튼 클릭이벤트 :
        binding.adminPostLeftIvBack.setOnClickListener {
      dismiss()
        }
