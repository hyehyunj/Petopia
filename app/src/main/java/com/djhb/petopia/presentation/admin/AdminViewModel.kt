...
//갤러리와 포토의 공유 뷰모델
class AdminSharedViewModel(private val adminRepository: GalleryRepository) :
    ViewModel() {
    private val user = LoginData.loginUser

    //갤러리 리스트
    private val _reportListLiveData = MutableLiveData<List<GalleryModel>>(listOf())
    val reportListLiveData: LiveData<List<GalleryModel>> = _reportListLiveData
...
//    val currentPhotoLiveData: LiveData<GalleryModel> = _currentPhotoListLiveData
    ...
    fun loadInitGalleryList() {
        viewModelScope.launch {
            val list = async {
                adminRepository.selectInitGalleryList(user)
            }
            val documents = list.await()
            lastSnapshot = documents[documents.size -1]
            val successList = adminRepository.convertToGalleryModel(documents)

            val imageUris = mutableListOf<StorageReference?>()

            for (galleryModel in successList) {
                galleryModel.imageUris.clear()
                imageUris.add(adminRepository.selectGalleryMainImages(galleryModel.uid))
            }

            for ((uriIndex, uri) in imageUris.withIndex()) {
                if(uri != null)
                    successList[uriIndex].imageUris.add(adminRepository.selectDownloadUri(uri))
            }
...
//            _galleryListLiveData.value = successList.toList()
    ...
    fun loadGalleryList() {
        viewModelScope.launch {
            val list = async {
                adminRepository.selectGalleryList(user, lastSnapshot)
            }
            val documents = list.await()
...
//            _galleryListLiveData.value = successList.toList()
    ...
    private fun saveGalleryList() {
        viewModelScope.launch {
            _currentPhotoListLiveData.value?.let { adminRepository.createGallery(it) }
        }
    }
        ...
        when (_layoutModeLiveData.value) {
            "ADD" -> {
                _currentPhotoListLiveData.value =
                    newPhotoList
                viewModelScope.launch {
                    adminRepository.createGallery(_currentPhotoListLiveData.value!!)
    ...
    fun updateRemoveGalleryList(mode: String) {
        when (mode) {
            ...
            "COMPLETE" -> {
                ...
                viewModelScope.launch {
                for(i in list) {
                    adminRepository.deleteGallery(i.uid)
                }}
